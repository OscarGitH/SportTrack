<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="utf-8">
    <title>SportTrack | Mon compte</title>
    <link rel="icon" type="image/png" href="img/Run_icon.png">
    <link rel="stylesheet" href="../views/index.css" media="screen" type="text/css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link href="https://fonts.googleapis.com/css2?family=Montserrat:ital,wght@0,300;0,400;0,700;1,400&display=swap" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css" rel="stylesheet">
</head>
<body>
    <header>
        <div class="menu">
            <a href="/"><img src="img/sporttrack.png" alt="SportTrack Logo"></a>
        </div>
    </header>
    
    <main>
        <div class="conteneur">
            <p class="explication">Erreur lors de la modification de l'utilisateur : l'adresse mail n'est pas au bon format ou un utilisateur avec le même mail existe.</p>
            <a class="signuptext" href="/my_account">Retour</a>
        </div>
    </main>
</body>
</html>